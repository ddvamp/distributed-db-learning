...
**Теорема FLP (Fischer, Lynch, Paterson, 85).** В Asynchronous Model даже в случае предусмотрения одного отказа никакой корректный детерменированный алгоритм консенсуса не способен гарантировать Termination во всех исполнениях
...
Теорема о partition говорит, что при наличии majority отказов теряется Agreement. Мы обязаны не завершиться. Нет safity $` \sim `$ deadlock. Теорема FLP говорит, что при возможности одного отказа, теряется Termination. Мы можем не завершиться. Нет liveness $` \sim `$ livelock
