...
Как поддержать рестарты? Путь реплики перед отправкой подтверждения координатору сохраняют состояние регистра на диск. При рестарте регистр можно восстановить, а состояние узла будет как после устранения partition-а. Аналогично можно было бы заменять отказавшие узлы, будто они со старта находились вне сети
